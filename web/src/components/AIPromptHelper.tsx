    ...
    <div className="rounded-lg border border-slate-700 bg-slate-900/40 overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-purple-300">Human responses preferred!</span>
          <span className="text-sm text-slate-400">But if you need AI assistance...</span>
        </div>
        {isOpen ? (
      ...
      {isOpen && (
        <div className="px-4 pb-4 space-y-4">
          ...
          <div className="rounded-lg bg-slate-800 border border-slate-700 p-3 space-y-2">
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide">Suggested AI Prompt</p>
            <hr className="border-slate-700" />
            <pre className="text-xs text-slate-300 whitespace-pre-wrap font-mono leading-relaxed">{PROMPT_RULES}</pre>
            <hr className="border-slate-700" />
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide">Your Flashcard Data:</p>
            <div className="bg-slate-900 rounded p-2 font-mono text-xs text-slate-300">
              <span className="text-slate-400">Question:</span> {question || 'Your question will appear here'}<br />
              <span className="text-slate-400">Answer:</span> {answer || 'Your answer will appear here'}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <button
              ...
              className="px-3 py-2 rounded-full border border-emerald-500 text-emerald-400 bg-emerald-950/30 hover:bg-emerald-950/60 text-sm font-semibold transition-colors"
            >
              Open in ChatGPT
            </button>
            <button
              ...
              className="px-3 py-2 rounded-full border border-sky-400 text-sky-400 bg-sky-950/30 hover:bg-sky-950/60 text-sm font-semibold transition-colors"
            >
              Open in Perplexity
            </button>
            <button
              ...
              className="px-3 py-2 rounded-full border border-amber-500 text-amber-400 bg-amber-950/30 hover:bg-amber-950/60 text-sm font-semibold transition-colors"
            >
              Open in Claude
            </button>
            <button
              type="button"
              onClick={handleCopy}
              className="px-3 py-2 rounded-full border border-slate-500 text-slate-300 bg-slate-800 hover:bg-slate-700 text-sm font-semibold transition-colors flex items-center justify-center gap-1"
            >
              {copied ? (
