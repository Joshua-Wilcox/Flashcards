-- Fix user_id type mismatch in admin RPC functions
-- The submitted_flashcards, user_stats, and module_stats tables all use TEXT for user_id,
-- but the RPC functions were defined with BIGINT, causing issues with n8n ingestion
-- which uses "n8n-ingest" as the user_id.

-- Update increment_user_approved_cards to use TEXT for user_id
CREATE OR REPLACE FUNCTION increment_user_approved_cards(
    user_id_param TEXT,  -- Changed from BIGINT to TEXT
    username_param TEXT,
...
-- Update admin_approve_flashcard to use TEXT for user_id
...
DECLARE
    user_id_var TEXT;  -- Changed from BIGINT to TEXT
    username_var TEXT;
