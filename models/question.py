...
def get_tags_for_question(question_id):
    """Get tags associated with a question. Legacy function - now uses comprehensive metadata fetch."""
    metadata = get_comprehensive_question_metadata([question_id])
    return metadata.get(question_id, {}).get('tags', [])

def get_topics_for_question(question_id):
    """Get topics associated with a question. Legacy function - now uses comprehensive metadata fetch."""
    metadata = get_comprehensive_question_metadata([question_id])
    return metadata.get(question_id, {}).get('topics', [])

def get_subtopics_for_question(question_id):
    """Get subtopics associated with a question. Legacy function - now uses comprehensive metadata fetch."""
    metadata = get_comprehensive_question_metadata([question_id])
    return metadata.get(question_id, {}).get('subtopics', [])
...
def get_question_with_distractors(module_name, topics=None, subtopics=None, tags=None, specific_question_id=None):
    """
    Get a complete question with all metadata and distractors in minimal database calls.
    
    Args:
        module_name: Name of the module
        topics: Optional list of topic filters
        subtopics: Optional list of subtopic filters  
        tags: Optional list of tag filters
        specific_question_id: Optional specific question to fetch
        
    Returns:
        {
            'question': {...},
            'metadata': {'topics': [...], 'subtopics': [...], 'tags': [...]},
            'manual_distractors': [...],
            'generated_distractors': [...],
            'pdfs': [...],
            'filters_applied': bool,
            'filters_relaxed': bool,
            'total_filtered_questions': int
        }
    """
    client = supabase_client.get_db()
    ...
    try:
        ...
        filter_conditions = []
        filter_params = [module_name]
        param_index = 2
        
        # Add topic filter condition
        if topics:
            filter_conditions.append(f"t.name = ANY(${param_index}::text[])")
            filter_params.append(topics)
            param_index += 1
        else:
            filter_conditions.append("$2::text[] IS NULL OR t.name = ANY($2::text[])")
            filter_params.insert(1, None)
            param_index += 1
            
        # Add subtopic filter condition
        if subtopics:
            filter_conditions.append(f"s.name = ANY(${param_index}::text[])")
            filter_params.append(subtopics)
            param_index += 1
        else:
            filter_conditions.append(f"${param_index}::text[] IS NULL OR s.name = ANY(${param_index}::text[])")
            filter_params.append(None)
            param_index += 1
            
        # Add tag filter condition  
        if tags:
            filter_conditions.append(f"tag.name = ANY(${param_index}::text[])")
            filter_params.append(tags)
            param_index += 1
        else:
            filter_conditions.append(f"${param_index}::text[] IS NULL OR tag.name = ANY(${param_index}::text[])")
            filter_params.append(None)
            param_index += 1
        
        # If specific question ID provided, override filtering
        if specific_question_id:
            # Simplified query for specific question
            questions_result = client.table('questions').select('id, question, answer, module_id').eq('id', specific_question_id).execute()
            if not questions_result.data:
                return result
            filtered_questions = questions_result.data
            result['total_filtered_questions'] = 1
        else:
            # Use optimized fallback filtering (no RPC needed - the fallback is efficient)
            filtered_questions = _get_filtered_questions_fallback(client, module_id, topics, subtopics, tags)
            result['total_filtered_questions'] = len(filtered_questions)
        
        # Handle empty results with filter relaxation
        if not filtered_questions and result['filters_applied']:
            result['filters_relaxed'] = True
            # Fall back to all questions in module
            questions_result = client.table('questions').select('id, question, answer, module_id').eq('module_id', module_id).execute()
            filtered_questions = questions_result.data if questions_result.data else []
            result['total_filtered_questions'] = len(filtered_questions)
        ...
        # Get comprehensive metadata for the selected question
        question_metadata = get_comprehensive_question_metadata([selected_question['id']])
        result['metadata'] = question_metadata.get(selected_question['id'], {'topics': [], 'subtopics': [], 'tags': []})
        
        # Get manual distractors and potential question distractors in single query
        distractors_result = _get_distractors_for_question(client, selected_question['id'], module_id)
        result['manual_distractors'] = distractors_result['manual']
        result['generated_distractors'] = distractors_result['questions']
        
        # Get PDFs for the question
        result['pdfs'] = get_pdfs_for_question(selected_question['id'])
        
        return result
        
    except Exception as e:
        print(f"Error in get_question_with_distractors: {e}")
        return result

def _get_filtered_questions_fallback(client, module_id, topics, subtopics, tags):
    """
    Fallback method for question filtering when RPC function is not available.
    Uses optimized nested queries with batching.
    """
    try:
        # Start with all questions in the module
        all_questions = client.table('questions').select('id, question, answer, module_id').eq('module_id', module_id).execute()
        if not all_questions.data:
            return []
        
        all_question_ids = [q['id'] for q in all_questions.data]
        
        # Apply topic filter if specified
        if topics:
            topic_question_ids = set()
            
            # Get topic IDs in batch
            topics_result = client.table('topics').select('id, name').in_('name', topics).execute()
            topic_id_map = {t['name']: t['id'] for t in topics_result.data} if topics_result.data else {}
            
            for topic_name in topics:
                if topic_name in topic_id_map:
                    topic_id = topic_id_map[topic_name]
                    # Get questions with this topic
                    topic_questions_result = client.table('question_topics').select('question_id').eq('topic_id', topic_id).execute()
                    if topic_questions_result.data:
                        topic_question_ids.update(q['question_id'] for q in topic_questions_result.data)
            
            if topic_question_ids:
                all_question_ids = [qid for qid in all_question_ids if qid in topic_question_ids]
            else:
                return []  # No questions match the topics
        
        # Apply subtopic filter if specified
        if subtopics and all_question_ids:
            subtopic_question_ids = set()
            
            # Get subtopic IDs in batch
            subtopics_result = client.table('subtopics').select('id, name').in_('name', subtopics).execute()
            subtopic_id_map = {s['name']: s['id'] for s in subtopics_result.data} if subtopics_result.data else {}
            
            for subtopic_name in subtopics:
                if subtopic_name in subtopic_id_map:
                    subtopic_id = subtopic_id_map[subtopic_name]
                    # Get questions with this subtopic
                    subtopic_questions_result = client.table('question_subtopics').select('question_id').eq('subtopic_id', subtopic_id).execute()
                    if subtopic_questions_result.data:
                        subtopic_question_ids.update(q['question_id'] for q in subtopic_questions_result.data)
            
            if subtopic_question_ids:
                all_question_ids = [qid for qid in all_question_ids if qid in subtopic_question_ids]
            else:
                return []  # No questions match the subtopics
        
        # Apply tag filter if specified
        if tags and all_question_ids:
            tag_question_ids = set()
            
            # Get tag IDs in batch
            tags_result = client.table('tags').select('id, name').in_('name', tags).execute()
            tag_id_map = {t['name']: t['id'] for t in tags_result.data} if tags_result.data else {}
            
            for tag_name in tags:
                if tag_name in tag_id_map:
                    tag_id = tag_id_map[tag_name]
                    # Get questions with this tag
                    tag_questions_result = client.table('question_tags').select('question_id').eq('tag_id', tag_id).execute()
                    if tag_questions_result.data:
                        tag_question_ids.update(q['question_id'] for q in tag_questions_result.data)
            
            if tag_question_ids:
                all_question_ids = [qid for qid in all_question_ids if qid in tag_question_ids]
            else:
                return []  # No questions match the tags
        
        # Return the filtered questions
        filtered_questions = [q for q in all_questions.data if q['id'] in all_question_ids]
...
def _get_distractors_for_question(client, question_id, module_id):
    """
    Get manual distractors and potential question distractors in optimized queries.
    ...
    """
    try:
        # Query 1: Get manual distractors
        manual_distractors_result = client.table('manual_distractors').select('id, distractor_text').eq('question_id', question_id).order('created_at').execute()
        manual_distractors = manual_distractors_result.data if manual_distractors_result.data else []
        
        # Query 2: Get potential question distractors with metadata using nested query
        potential_distractors_result = client.table('questions').select('''
            id, answer,
            question_topics!left(topics!inner(name)),
            question_subtopics!left(subtopics!inner(name)),
            question_tags!left(tags!inner(name))
        ''').eq('module_id', module_id).neq('id', question_id).limit(50).execute()  # Limit to reduce data size
        
        # Process the nested results to extract metadata
        question_distractors = []
        for q in potential_distractors_result.data if potential_distractors_result.data else []:
            distractor = {
                'id': q['id'],
                'answer': q['answer'],
                'topics': sorted(list(set([rel['topics']['name'] for rel in q['question_topics'] if rel.get('topics')]))),
                'subtopics': sorted(list(set([rel['subtopics']['name'] for rel in q['question_subtopics'] if rel.get('subtopics')]))),
                'tags': sorted(list(set([rel['tags']['name'] for rel in q['question_tags'] if rel.get('tags')])))
            }
            question_distractors.append(distractor)
        
        return {
            'manual': manual_distractors,
            'questions': question_distractors
        }
        
    except Exception as e:
        print(f"Error in _get_distractors_for_question: {e}")
...
def score_and_select_distractors(question_metadata, potential_distractors, manual_distractors, num_needed):
    """
    Score and select best distractors from pre-fetched candidates.
    
    Args:
        question_metadata: Current question's metadata 
        potential_distractors: List of potential question distractors with metadata
        manual_distractors: List of manual distractors
        num_needed: Number of distractors needed
        
    Returns:
        List of selected distractors with answer, id, type, and metadata
    """
    selected_distractors = []
    
    # Add manual distractors first (they get priority)
    for distractor in manual_distractors:
        if len(selected_distractors) < num_needed:
            selected_distractors.append({
                'answer': distractor['distractor_text'],
                'id': None,  # Manual distractors don't have question IDs
                'type': 'manual_distractor',
                'metadata': distractor['id']  # Store the manual distractor ID
            })
    
    # If we need more distractors, score and select from questions
    remaining_needed = num_needed - len(selected_distractors)
    if remaining_needed > 0 and potential_distractors:
        scored_distractors = []
        
        for distractor in potential_distractors:
            # Calculate similarity score using existing algorithm
            score = calculate_distractor_similarity_score(
                question_metadata['topics'],
                question_metadata['subtopics'], 
                question_metadata['tags'],
                distractor['topics'],
                distractor['subtopics'],
                distractor['tags']
            )
            scored_distractors.append((distractor, score))
        
        # Sort by score (highest first) and select the best ones
        scored_distractors.sort(key=lambda x: x[1], reverse=True)
        best_distractors = scored_distractors[:remaining_needed]
        
        for distractor, score in best_distractors:
            selected_distractors.append({
